...
inline fun <reified EVENT> ViewModel.observeEvent(
    vararg tags: String,
    noinline observer: (EVENT) -> Unit
) {
    val o = Observer<EVENT> {
    ...
    tags.forEach {
        eventObservable<EVENT>(it).observeForever(o)
    }
}

inline fun <reified EVENT> ViewModel.observeEventSticky(
    vararg tags: String,
    noinline observer: (EVENT) -> Unit
) {
    val o = Observer<EVENT> {
    ...
    tags.forEach {
        eventObservable<EVENT>(it).observeStickyForever(o)
    }
}