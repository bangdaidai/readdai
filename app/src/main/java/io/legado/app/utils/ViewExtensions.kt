...
fun View.applyTint(
    @ColorInt color: Int,
    isDark: Boolean = io.legado.app.utils.ColorUtils.isColorLight(color)
) {
...
fun TextView.setHtml(html: String, imageGetter: GlideImageGetter? = null, textViewTagHandler: TextViewTagHandler? = null, imgOnLongClickListener: (source: String) -> Unit, imgOnClickListener: (click: String) -> Unit) {
    ...
    val imageSpans = spanned.getSpans(0, spanned.length, android.text.style.ImageSpan::class.java)
...
fun TextView.setMarkdown(markwon: Markwon, spanned: android.text.Spanned, imgOnLongClickListener: (source: String) -> Unit) {
