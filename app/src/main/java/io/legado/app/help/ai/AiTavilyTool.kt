...
object AiTavilyTool {
    ...
    private suspend fun search(arguments: JSONObject?): String = withContext(IO) {
        ...
        repeat(MAX_RETRY_COUNT + 1) { attempt ->
            try {
                return@withContext executeSearchWithTimeout(query, arguments, attempt)
            } catch (e: Exception) {
                lastException = e
                if (attempt < MAX_RETRY_COUNT) {
                    ...
                    io.legado.app.help.ai.AiLogManager.log(
                        ...
                        "TavilySearch",
                        "搜索失败，第 ${attempt + 1} 次重试: ${e.message}"
                    )
        ...
        // 所有重试都失败
        errorJson("Tavily 搜索失败: ${lastException?.message ?: "未知错误"}")
    }
    
    private suspend fun executeSearchWithTimeout(
        query: String,
        arguments: JSONObject?,
        attempt: Int
    ): String {
        val result = withTimeoutOrNull(REQUEST_TIMEOUT_SECONDS.seconds) {
            performSearchRequest(query, arguments, attempt)
        }
        
        return result ?: errorJson("Tavily 搜索超时 (${REQUEST_TIMEOUT_SECONDS}秒)")
    }
    
    private suspend fun performSearchRequest(
        query: String,
        arguments: JSONObject?,
        attempt: Int
    ): String {
        ...
        val response = okHttpClient.newCallResponse {
            url(normalizeUrl(AppConfig.aiTavilyBaseUrl.orEmpty()))
            addHeader("Accept", "application/json")
            addHeader("Authorization", "Bearer ${AppConfig.aiTavilyApiKey.orEmpty()}")
