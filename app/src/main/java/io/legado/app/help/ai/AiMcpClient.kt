...
object AiMcpClient {
    ...
    private suspend fun listTools(server: AiMcpServerConfig): List<McpToolDescriptor> {
        ...
        io.legado.app.help.ai.AiLogManager.log(
            io.legado.app.help.ai.AiLogManager.LogLevel.INFO,
            "MCP Client",
            "成功获取 ${tools.size} 个 MCP 工具: ${server.name}"
        )
    ...
    private suspend fun ensureSession(server: AiMcpServerConfig): SessionState {
        ...
        val initializeBody = jsonRpcRequest(
            ...
            params = JSONObject().apply {
                ...
                put(
                    ...
                    JSONObject().apply {
                        put("name", "ReadDai")
                        put("version", try { io.legado.app.BuildConfig.VERSION_NAME } catch (e: Exception) { "1.0.0" })
                    }
                )
        ...
        val response = withTimeoutOrNull(REQUEST_TIMEOUT_SECONDS.seconds) {
        ...
        if (response == null) {
            throw IllegalStateException("MCP 服务器连接超时 (${REQUEST_TIMEOUT_SECONDS}秒): ${server.endpoint}")
        }
    ...
    private suspend fun postJsonRpc(
        ...
        return runCatching {
            ...
            if (response == null) {
                throw IllegalStateException("MCP 请求超时 (${REQUEST_TIMEOUT_SECONDS}秒)")
            }
            ...
            io.legado.app.help.ai.AiLogManager.log(
                io.legado.app.help.ai.AiLogManager.LogLevel.ERROR,
                "MCP Client",
                "MCP 请求失败，尝试重新建立会话: ${exception.message}"
            )
            ...
            val retryResponse = withTimeoutOrNull(REQUEST_TIMEOUT_SECONDS.seconds) {
                okHttpClient.newCallResponse {
                    url(server.endpoint)
                    addHeader("Accept", "application/json, text/event-stream")
                    addHeader("Content-Type", "application/json")
            ...
            if (retryResponse == null) {
                throw IllegalStateException("MCP 重试请求超时 (${REQUEST_TIMEOUT_SECONDS}秒)")
            }
            
            retryResponse.use { rawResponse ->
                readJsonRpcResponse(rawResponse, requestId)
            }
        }.getOrThrow()
    }
