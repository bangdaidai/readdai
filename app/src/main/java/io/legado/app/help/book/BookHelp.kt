...
object BookHelp {
    ...
    suspend fun updateCacheFolder(oldBook: Book, newBook: Book) {
        ...
        // 检查并移动封面文件
        moveCoverFileIfNeeded(oldBook, newBook, oldFolderPath, newFolderPath)
     ...
     */
    private suspend fun moveCoverFileIfNeeded(oldBook: Book, newBook: Book, oldFolderPath: String, newFolderPath: String) {
        try {
            val coverUrl = oldBook.getDisplayCover()
            if (coverUrl.isNullOrEmpty()) return
            
            // 只处理本地文件封面
            if (coverUrl.startsWith("file://")) {
                val coverFile = File(URI(coverUrl))
                
                // 检查封面文件是否在旧文件夹内
                if (coverFile.exists() && coverFile.absolutePath.startsWith(oldFolderPath)) {
                    // 创建新文件夹（如果不存在）
                    ...
                    // 计算封面文件在新文件夹中的路径
                    val relativePath = coverFile.absolutePath.substring(oldFolderPath.length)
                    val newCoverPath = newFolderPath + relativePath
                    val newCoverFile = File(newCoverPath)
                    
                    // 确保目标目录存在
                    newCoverFile.parentFile?.mkdirs()
                    
                    // 移动封面文件
                    if (coverFile.copyTo(newCoverFile, overwrite = true).exists()) {
                        // 更新Book实体中的封面路径
                        val newCoverUrl = newCoverFile.toURI().toString()
                        
                        // 检查是否是自定义封面
                        if (oldBook.customCoverUrl == coverUrl) {
                            newBook.customCoverUrl = newCoverUrl
                        }
                        ...
                        coroutineScope {
                            launch {
                                try {
                                    val readingMemory = appDb.readingMemoryDao.getByBookUrl(oldBook.bookUrl)
                                    if (readingMemory != null && readingMemory.coverUrl == coverUrl) {
                                    ...
                                    AppLog.put("更新阅读记忆封面路径失败: ${e.localizedMessage}", e)
                                }
                            }
                        }
                    }
            ...
            AppLog.put("移动封面文件失败: ${e.localizedMessage}", e)
        }
    }
