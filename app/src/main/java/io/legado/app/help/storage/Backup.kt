...
object Backup {
    ...
    private suspend fun backup(context: Context, path: String?) {
        ...
        writeListToJson(appDb.readSessionDao.allShow, "readRecord.json", backupPath)
        writeListToJson(appDb.readSessionDao.getAll(), "readSession.json", backupPath)
        writeListToJson(appDb.readingMemoryDao.all, "readingMemory.json", backupPath)
        ...
        writeListToJson(appDb.bookAnnotationDao.all, "bookAnnotation.json", backupPath)
        writeListToJson(appDb.bookReviewDao.getAll(), "bookReview.json", backupPath)
        writeListToJson(appDb.bookTagDao.getAll(), "bookTag.json", backupPath)
        writeListToJson(appDb.bookTagGroupDao.getAll(), "bookTagGroup.json", backupPath)
        writeListToJson(withContext(IO) { appDb.bookTagRelationDao.getAll() }, "bookTagRelation.json", backupPath)
        writeListToJson(withContext(IO) { appDb.tagMappingDao.getAll() }, "tagMapping.json", backupPath)
        writeListToJson(appDb.excludedTagDao.getAllSync(), "excludedTag.json", backupPath)
        writeListToJson(withContext(IO) { appDb.removedAutoTagDao.getAll() }, "removedAutoTag.json", backupPath)
        writeListToJson(appDb.bookProtagonistDao.getAll(), "bookProtagonist.json", backupPath)
        
        // 备份AI服务商配置
        try {
            val aiDatabase = AiDatabase.getInstance(context)
            val aiProviders = aiDatabase.aiDao().getAllProviders()
            if (aiProviders.isNotEmpty()) {
                writeListToJson(aiProviders, "aiProviders.json", backupPath)
                LogUtils.d(TAG, "AI服务商配置备份完成，数量: ${aiProviders.size}")
            } else {
                LogUtils.d(TAG, "无AI服务商配置需要备份")
            }
        } catch (e: Exception) {
        ...
        currentCoroutineContext().ensureActive()
        backupThemeBgImages(backupPath)
        currentCoroutineContext().ensureActive()
    ...
    private fun backupThemeBgImages(backupPath: String) {
        val themeBgFiles = arrayListOf<File>()
        appCtx.externalFiles.getFile(PreferKey.bgImage).listFiles()?.forEach { file ->
            themeBgFiles.add(file)
        }
        appCtx.externalFiles.getFile(PreferKey.bgImageN).listFiles()?.forEach { file ->
            themeBgFiles.add(file)
        }
        if (themeBgFiles.isNotEmpty()) {
            val themeBgDir = File(backupPath, "themeBg")
            themeBgDir.mkdirs()
            themeBgFiles.forEach { file ->
                file.copyTo(File(themeBgDir, file.name), overwrite = true)
            }
            LogUtils.d(TAG, "主题背景图片备份完成，数量: ${themeBgFiles.size}")
        }
    }
