...
fun Book.toReplaceBook(): ReplaceBook {
    return ReplaceBook(
        bookUrl = bookUrl,
        origin = origin,
...
fun Book.getDisplayName(): String {
    val replaceRules = ContentProcessor.get(this).getTitleReplaceRules()
    if (replaceRules.isEmpty() || !getUseReplaceRule()) {
        return name
    }
        ...
        if (rule.pattern.isNotEmpty()) {
            try {
                val result = if (rule.isRegex) {
                    displayName.replace(rule.regex, rule.replacement)
                } else {
                    displayName.replace(rule.pattern, rule.replacement)
                }
...
fun Book.getDisplayDurChapterTitle(): String {
    val replaceRules = ContentProcessor.get(this).getTitleReplaceRules()
    if (replaceRules.isEmpty() || !getUseReplaceRule() || durChapterTitle.isNullOrEmpty()) {
        return durChapterTitle ?: ""
    }
    var displayTitle = durChapterTitle!!
    replaceRules.forEach { rule ->
        if (rule.pattern.isNotEmpty()) {
            try {
                val result = if (rule.isRegex) {
