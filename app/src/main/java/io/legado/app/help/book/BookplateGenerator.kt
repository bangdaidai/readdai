...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .top-list { padding: 0 6px; }
  .top-item {
    display: flex; align-items: center; padding: 8px 0;
    border-bottom: 1px solid rgba(160,140,120,0.12);
  }
  .top-rank {
    width: 24px; height: 24px; border-radius: 50%; text-align: center; line-height: 24px;
    font-size: 12px; font-weight: 700; color: #fff; margin-right: 12px;
    flex-shrink: 0;
  }
  .top-rank.r1 { background: #c9a96e; }
  .top-rank.r2 { background: #a0a0a0; }
  .top-rank.r3 { background: #b8864e; }
  .top-rank.r4, .top-rank.r5 { background: rgba(140,120,100,0.4); }
  .top-name { flex: 1; font-size: clamp(11px, 2.8vw, 14px); color: #4a3f35; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .top-time { font-size: clamp(9px, 2vw, 12px); opacity: 0.5; margin-left: 8px; flex-shrink: 0; }
...
<body>
  ...
  <hr class="divider">
  <div class="section">
    <div class="section-title">Top 5 Most Read / 最爱阅读</div>
    <div class="top-list">
      <div class="top-item">
        <div class="top-rank r1">1</div>
        <div class="top-name">{{top1Name}}</div>
        <div class="top-time">{{top1Time}}</div>
      </div>
      <div class="top-item">
        <div class="top-rank r2">2</div>
        <div class="top-name">{{top2Name}}</div>
        <div class="top-time">{{top2Time}}</div>
      </div>
      <div class="top-item">
        <div class="top-rank r3">3</div>
        <div class="top-name">{{top3Name}}</div>
        <div class="top-time">{{top3Time}}</div>
      </div>
      <div class="top-item">
        <div class="top-rank r4">4</div>
        <div class="top-name">{{top4Name}}</div>
        <div class="top-time">{{top4Time}}</div>
      </div>
      <div class="top-item">
        <div class="top-rank r5">5</div>
        <div class="top-name">{{top5Name}}</div>
        <div class="top-time">{{top5Time}}</div>
      </div>
    </div>
  </div>
  <div class="footer">
    <p>LEGADO · 阅读统计</p>
    <p>Generated by Legado Reading App</p>
  </div>
</body>
</html>
    """.trimIndent()
