...
object HighlightRuleStore {
        ...
        return rules.map { rule ->
            ...
            val base = if (builtin != null && shouldRefreshBuiltin(safeRule)) {
                builtin.copy(
                    ...
                    nineLeftX = safeRule.nineLeftX.takeIf { it != 0.5f } ?: builtin.nineLeftX,
                    nineRightX = safeRule.nineRightX.takeIf { it != 0.5f } ?: builtin.nineRightX,
                    nineTopY = safeRule.nineTopY.takeIf { it != 0.5f } ?: builtin.nineTopY,
                    nineBottomY = safeRule.nineBottomY.takeIf { it != 0.5f } ?: builtin.nineBottomY,
    ...
    fun sanitizeRule(
        ...
        return HighlightRule(
            ...
            nineLeftX = runCatching { rule.nineLeftX }.getOrDefault(0.5f).coerceIn(0.02f, 0.98f),
            nineRightX = runCatching { rule.nineRightX }.getOrDefault(0.5f).coerceIn(0.02f, 0.98f),
            nineTopY = runCatching { rule.nineTopY }.getOrDefault(0.5f).coerceIn(0.02f, 0.98f),
            nineBottomY = runCatching { rule.nineBottomY }.getOrDefault(0.5f).coerceIn(0.02f, 0.98f),
