...
object HighlightRuleStore {
    ...
    fun load(context: Context): MutableList<HighlightRule> {
        ...
        val rules = if (stored.isNullOrBlank()) {
            // 首次加载，直接初始化预设规则
            val defaults = createDefaultRules(context)
            save(context, defaults)
            defaults.toMutableList()
        } else {
            val parsed = GSON.fromJsonArray<HighlightRule>(stored).getOrNull()?.toMutableList()
            if (parsed != null) {
                val normalized = normalizeRules(parsed, context)
        ...
        return rules.map { rule ->
            ...
            val base = if (builtin != null && shouldRefreshBuiltin(safeRule)) {
                builtin.copy(
                    ...
                    bgPaddingStart = safeRule.bgPaddingStart.takeIf { it != 0f } ?: builtin.bgPaddingStart,
                    bgPaddingEnd = safeRule.bgPaddingEnd.takeIf { it != 0f } ?: builtin.bgPaddingEnd,
                    bgPaddingTop = safeRule.bgPaddingTop.takeIf { it != 0f } ?: builtin.bgPaddingTop,
                    bgPaddingBottom = safeRule.bgPaddingBottom.takeIf { it != 0f } ?: builtin.bgPaddingBottom,
                    nineLeftX = safeRule.nineLeftX.takeIf { it != 1f / 3f } ?: builtin.nineLeftX,
                    nineRightX = safeRule.nineRightX.takeIf { it != 2f / 3f } ?: builtin.nineRightX,
                    nineTopY = safeRule.nineTopY.takeIf { it != 1f / 3f } ?: builtin.nineTopY,
                    nineBottomY = safeRule.nineBottomY.takeIf { it != 2f / 3f } ?: builtin.nineBottomY,
                    nineStretchMode = safeRule.nineStretchMode.takeIf { it != 0 } ?: builtin.nineStretchMode,
    ...
    fun sanitizeRule(
        ...
        return HighlightRule(
            ...
            bgPaddingStart = runCatching { rule.bgPaddingStart }.getOrDefault(0f).coerceIn(-50f, 50f),
            bgPaddingEnd = runCatching { rule.bgPaddingEnd }.getOrDefault(0f).coerceIn(-50f, 50f),
            bgPaddingTop = runCatching { rule.bgPaddingTop }.getOrDefault(0f).coerceIn(-50f, 50f),
            bgPaddingBottom = runCatching { rule.bgPaddingBottom }.getOrDefault(0f).coerceIn(-50f, 50f),
            nineLeftX = runCatching { rule.nineLeftX }.getOrDefault(1f / 3f).coerceIn(0.02f, 0.98f),
            nineRightX = runCatching { rule.nineRightX }.getOrDefault(2f / 3f).coerceIn(0.02f, 0.98f),
            nineTopY = runCatching { rule.nineTopY }.getOrDefault(1f / 3f).coerceIn(0.02f, 0.98f),
            nineBottomY = runCatching { rule.nineBottomY }.getOrDefault(2f / 3f).coerceIn(0.02f, 0.98f),
            nineStretchMode = runCatching { rule.nineStretchMode }.getOrDefault(0).coerceIn(0, 2),
            scope = scope,
