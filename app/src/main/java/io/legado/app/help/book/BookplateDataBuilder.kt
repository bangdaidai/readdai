...
object BookplateDataBuilder {
    ...
    suspend fun build(book: Book): BookplateData = withContext(Dispatchers.IO) {
        ...
        val data = coroutineScope {
            val annotationsD = async { appDb.bookAnnotationDao.getByBook(book.name, book.author) }
            val reviewsD = async { appDb.bookReviewDao.getReviewByBookUrl(book.bookUrl) }
            val readSessionD = async {
            ...
            val protagonistsD = async { appDb.bookProtagonistDao.getByBook(book.bookUrl).map { it.name } }
            val tagRelationsD = async { appDb.bookTagRelationDao.getRelationsByBook(book.bookUrl) }

            val annotations = annotationsD.await()
            val reviews = reviewsD.await()
            val readSessionTotal = readSessionD.await()
            val protagonists = protagonistsD.await()
            val tagRelations = tagRelationsD.await()
            ...
            BookplateLogger.log("DATA", "书摘:${annotations.size} 书评:${reviews.size} 时长:${readSessionTotal}ms 主角:${protagonists.size} 标签:${tags.size}")
            ...
            BookplateData(
                ...
                latestAnnotation = latestAnno?.bookText?.take(200) ?: "",
                latestAnnotationNote = latestAnno?.content ?: "",
                latestAnnotationChapter = latestAnno?.chapterName ?: "",
        ...
        BookplateLogger.log("DATA", "数据构建完成 (Book): ${data.bookName}, 进度=${data.readingProgress}")
        data
    }
    ...
    suspend fun build(memory: ReadingMemory): BookplateData = withContext(Dispatchers.IO) {
        ...
        val data = coroutineScope {
            val annotationsD = async { appDb.bookAnnotationDao.getByBook(memory.bookName, memory.bookAuthor) }
            val reviewsD = async { appDb.bookReviewDao.getReviewByBookUrl(memory.bookUrl) }
            val readSessionD = async {
            ...
            BookplateLogger.log("DATA", "书摘:${annotations.size} 书评:${reviews.size} 时长:${readSessionTotal}ms")
        ...
        BookplateLogger.log("DATA", "数据构建完成 (Memory): ${data.bookName}, 进度=${data.readingProgress}")
        data
    }
