...
class VideoPlayService : BaseService() {
    ...
    /**
     * 每隔60秒自动保存一次播放进度作为安全网
     */
    private fun upPlayProgress() {
        upPlayProgressJob?.cancel()
        upPlayProgressJob = serviceScope.launch {
            var lastSavePos = -1
            var lastReadTimeSave = 0L
            while (isActive) {
                val durP = playerView.getCurrentPositionWhenPlaying()
                postEvent(EventBus.VIDEO_PROGRESS, durP.toInt())
                postEvent(EventBus.VIDEO_SIZE, playerView.duration.toInt())
                upMediaSessionPlaybackState(PlaybackStateCompat.STATE_PLAYING)
                pause = false
                if (durP.toInt() != lastSavePos) {
                    lastSavePos = durP.toInt()
                    VideoPlay.saveRead(lastSavePos)
                }
                val now = System.currentTimeMillis()
                if (now - lastReadTimeSave >= 5 * 60 * 1000L) {
                    lastReadTimeSave = now
                    VideoPlay.upReadTime()
                }
                delay(60000)
            }
