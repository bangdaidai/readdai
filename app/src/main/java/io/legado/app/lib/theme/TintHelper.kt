    ...
    fun setTintAuto(
        ...
        if (!isBg) {
            when (view) {
                ...
                is SearchView -> {
                    // 设置所有图标的颜色为主题自定义的标题栏文字图标颜色
                    val titleBarTextIconColor = io.legado.app.lib.theme.ThemeStore.titleBarTextIconColor(view.context)
                    val iconIdS =
                        intArrayOf(
                    ...
                    for (iconId in iconIdS) {
                        ...
                        if (icon != null) {
                            setTint(icon, titleBarTextIconColor)
                        }
                    }
                    // 设置搜索框内文字的颜色为主题自定义的标题栏文字图标颜色
                    val searchEditText = view.findViewById<EditText>(androidx.appcompat.R.id.search_src_text)
                    if (searchEditText != null) {
                        searchEditText.setTextColor(titleBarTextIconColor)
                        // 添加 TextWatcher 监听文本变化，确保清除按钮显示时也能应用颜色
                        // 使用 tag 避免重复添加
                        if (searchEditText.getTag() == null) {
                            searchEditText.setTag("tint_added")
