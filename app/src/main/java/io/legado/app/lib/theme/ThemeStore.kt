...
private constructor(private val mContext: Context) : ThemeStoreInterface {
    ...
    override fun textColorOther(@ColorInt color: Int): ThemeStore {
        mEditor.putInt(ThemeStorePrefKeys.KEY_TEXT_COLOR_OTHER, color)
        return this
    }

    override fun textColorOtherRes(@ColorRes colorRes: Int): ThemeStore {
        return textColorOther(ContextCompat.getColor(mContext, colorRes))
    }

    override fun textColorOtherAttr(@AttrRes colorAttr: Int): ThemeStore {
        return textColorOther(ThemeUtils.resolveColor(mContext, colorAttr))
    }
    ...
    override fun dividerColor(@ColorInt color: Int): ThemeStore {
        mEditor.putInt(ThemeStorePrefKeys.KEY_DIVIDER, color)
        return this
    }
    ...
    override fun apply() {
        mEditor.putLong(ThemeStorePrefKeys.VALUES_CHANGED, System.currentTimeMillis())
            ...
            .apply()
        // 更新所有相关的静态变量，确保它们获取最新的值
        accentColor = accentColor()
        // 发送主题更改通知，让所有使用主题颜色的视图重新加载
        io.legado.app.utils.postEvent(io.legado.app.constant.EventBus.THEME_CHANGED, "")
    }
    ...
    companion object {
        ...
        @CheckResult
        @ColorInt
        fun textColorOther(context: Context): Int {
            return prefs(context).getInt(
                ThemeStorePrefKeys.KEY_TEXT_COLOR_OTHER,
                ThemeUtils.resolveColor(context, android.R.attr.textColorPrimary)
            )
        }
        ...
        @CheckResult
        @ColorInt
        fun titleBarTextIconColor(context: Context): Int {
            return prefs(context).getInt(
                if (context.resources.configuration.isNightMode) {
                    "colorTitleBarTextIconNight"
                } else {
                    "colorTitleBarTextIcon"
                },
                textColorPrimaryInverse(context)
            )
        }
        ...
        @CheckResult
    @ColorInt
    fun backgroundCard(context: Context = appCtx): Int {
        return prefs(context).getInt(
            ThemeStorePrefKeys.KEY_BACKGROUND_CARD,
            ThemeUtils.resolveColor(context, android.R.attr.colorBackground)
        )
    }
    ...
    @CheckResult
    @ColorInt
    fun dividerColor(context: Context = appCtx): Int {
        return prefs(context).getInt(
            ThemeStorePrefKeys.KEY_DIVIDER,
            context.getCompatColor(io.legado.app.R.color.divider)
        )
    }
