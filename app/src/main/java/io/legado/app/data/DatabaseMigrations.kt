...
object DatabaseMigrations {
    ...
    private val migration_18_19 = object : Migration(18, 19) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            db.execSQL("INSERT INTO readRecordNew(androidId, bookName, readTime) select 'unknown' as androidId, bookName, readTime from readRecord")
    ...
    val migration_88_89 = object : Migration(88, 89) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // 添加lastRead字段到readRecord表
            db.execSQL("ALTER TABLE readRecord ADD lastRead INTEGER NOT NULL DEFAULT 0")
            
            // 更新现有记录，将lastRead设置为当前时间戳
            db.execSQL("UPDATE readRecord SET lastRead = strftime('%s', 'now') * 1000 WHERE lastRead = 0")
        }
    }
    ...
    val migration_89_90 = object : Migration(89, 90) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // 创建readSession表
            db.execSQL("""
                CREATE TABLE IF NOT EXISTS `readSession` (
            ...
            // 创建索引
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_readSession_bookName` ON `readSession` (`bookName`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_readSession_deviceId` ON `readSession` (`deviceId`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_readSession_startTime` ON `readSession` (`startTime`)")
        }
    }

    val migration_93_94 = object : Migration(93, 94) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // 添加wordCount和kind字段到readingMemory表
            db.execSQL("ALTER TABLE readingMemory ADD wordCount TEXT")
            db.execSQL("ALTER TABLE readingMemory ADD kind TEXT")
        }
    }

    val MIGRATION_96_97 = object : Migration(96, 97) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // 为bookTags表添加唯一索引，确保标签名称唯一
            db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_bookTags_name` ON `bookTags` (`name`)")
            
            // 清理重复标签的数据
            // 1. 创建临时表存储需要保留的标签（每个名称保留ID最小的）
            db.execSQL("""
                CREATE TEMPORARY TABLE `temp_bookTags` AS 
                SELECT MIN(id) as id, name, color, createTime, updateTime 
                FROM bookTags 
                GROUP BY name
            """)
            
            // 2. 删除所有重复标签
            db.execSQL("DELETE FROM bookTags WHERE id NOT IN (SELECT id FROM temp_bookTags)")
            
            // 3. 更新BookTagRelation表，将所有关联指向保留的标签
            db.execSQL("""
                UPDATE bookTagRelation 
                SET tagId = (
                    SELECT temp.id 
                    FROM temp_bookTags temp 
                    JOIN bookTags bt ON temp.name = bt.name 
                    WHERE bt.id = bookTagRelation.tagId
                )
                WHERE tagId IN (
                    SELECT bt.id 
                    FROM bookTags bt 
                    WHERE bt.id NOT IN (SELECT id FROM temp_bookTags)
                )
            """)
            
            // 4. 删除临时表
            db.execSQL("DROP TABLE temp_bookTags")
        }
    }
    ...
    @Suppress("ClassName")
    class Migration_97_98 : AutoMigrationSpec {
    }
    ...
    val migration_98_99 = object : Migration(98, 99) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // 修改readSession表，添加缺失的字段
            db.execSQL("ALTER TABLE readSession ADD COLUMN author TEXT DEFAULT ''")
            db.execSQL("ALTER TABLE readSession ADD COLUMN bookUrl TEXT DEFAULT ''")
            db.execSQL("ALTER TABLE readSession ADD COLUMN words INTEGER DEFAULT 0")
            
            // 删除readRecordSession表（如果存在）
            db.execSQL("DROP TABLE IF EXISTS readRecordSession")
            
            // 更新索引
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_readSession_bookName_author` ON `readSession` (`bookName`, `author`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_readSession_bookUrl` ON `readSession` (`bookUrl`)")
        }
    }
    ...
    val migration_99_100 = object : Migration(99, 100) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // 创建bookProtagonists表
            db.execSQL("""
                CREATE TABLE IF NOT EXISTS `bookProtagonists` (
                    `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    ...
    val migration_105_106 = object : Migration(105, 106) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `bookplateTemplates` ADD COLUMN `templateType` INTEGER NOT NULL DEFAULT 0")
        }
    }
