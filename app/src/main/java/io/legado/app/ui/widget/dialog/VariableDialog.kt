...
class VariableDialog() : BaseDialogFragment(R.layout.dialog_variable, true),
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 设置中间 LinearLayout 的背景色为主题自定义的背景色
        if (binding.root is android.widget.FrameLayout) {
            val frameLayout = binding.root as android.widget.FrameLayout
            if (frameLayout.childCount > 0) {
                val linearLayout = frameLayout.getChildAt(0)
                if (linearLayout is android.widget.LinearLayout) {
        ...
        binding.toolBar.menu.findItem(R.id.menu_save)?.icon?.setTint(textColor)
