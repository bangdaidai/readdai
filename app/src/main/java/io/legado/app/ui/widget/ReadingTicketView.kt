...
class ReadingTicketView @JvmOverloads constructor(
    ...
    init {
        ...
        etReview.isFocusable = false
        etReview.isFocusableInTouchMode = false
    }
    ...
    fun setTicket(ticket: ReadingTicket, book: io.legado.app.data.entities.Book) {
        ...
        // 从 BookReview 表加载书评（仅展示）
        CoroutineScope(Dispatchers.IO).launch {
            val reviews = io.legado.app.data.appDb.bookReviewDao.getReviewByBookUrlSync(book.bookUrl)
            val reviewContent = reviews.firstOrNull()?.reviewContent ?: ""
            CoroutineScope(Dispatchers.Main).launch {
                etReview.setText(reviewContent)
            }
        }
    }
