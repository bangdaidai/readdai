...
class ReadMenu @JvmOverloads constructor(
    ...
    val fabNightTheme: FloatingActionButton get() = binding.fabNightTheme
    ...
    private fun initView(reset: Boolean = false) = binding.run {
        ...
        // 获取标题栏文字图标颜色
        val titleBarTextColor = io.legado.app.lib.theme.ThemeStore.titleBarTextIconColor(context)
        if (immersiveMenu) {
            val lightTextColor = ColorUtils.withAlpha(ColorUtils.lightenColor(titleBarTextColor), 0.75f)
            titleBar.setTextColor(titleBarTextColor)
            titleBar.setBackgroundColor(bgColor)
            titleBar.setColorFilter(titleBarTextColor)
            ...
            titleBar.setBackgroundColor(bgColor)
            titleBar.setColorFilter(titleBarTextColor)
            tvChapterName.setTextColor(titleBarTextColor)
            tvChapterUrl.setTextColor(titleBarTextColor)
        }
    ...
    fun refreshMenuColorFilter() {
        val titleBarTextColor = io.legado.app.lib.theme.ThemeStore.titleBarTextIconColor(context)
        if (immersiveMenu) {
            binding.titleBar.setColorFilter(titleBarTextColor)
        }
    }
    ...
    private fun bindEvent() = binding.run {
        ...
        //AI
        fabAi.setOnClickListener { callBack.onClickAiButton() }
