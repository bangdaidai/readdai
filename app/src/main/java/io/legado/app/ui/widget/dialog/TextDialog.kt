...
class TextDialog() : BaseDialogFragment(R.layout.dialog_text_view) {
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.toolBar.menu.findItem(R.id.menu_close)?.icon?.setTint(titleBarTextIconColor)
        // 设置菜单项文字颜色为标题栏文字图标颜色
        for (i in 0 until binding.toolBar.menu.size()) {
            val item = binding.toolBar.menu.getItem(i)
        ...
        binding.textView.setBackgroundColor(ThemeStore.backgroundColor(requireContext()))
        arguments?.let {
