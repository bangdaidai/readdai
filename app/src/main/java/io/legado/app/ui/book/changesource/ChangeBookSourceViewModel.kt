...
open class ChangeBookSourceViewModel(application: Application) : BaseViewModel(application) {
    ...
    var name: String = ""
    var author: String = ""
    var bookType: Int = BookType.text
    ...
    open fun initData(arguments: Bundle?, book: Book?, fromReadBookActivity: Boolean) {
        arguments?.let { bundle ->
            ...
            bundle.getInt("bookType", BookType.text).let {
                bookType = it
            }
    ...
    fun startSearch() {
        execute {
            ...
            val isVideo = bookType and BookType.video != 0
            if (isVideo) {
                bookSourceParts.addAll(appDb.bookSourceDao.getEnabledPartByType(BookSourceType.video))
    ...
    private suspend fun search(source: BookSource) {
        ...
        val resultBooks = WebBook.searchBookAwait(
            ...
            filter = { fName, fAuthor, _ ->
                if (isVideo) {
                    fName.contains(name) && (!checkAuthor || fAuthor.contains(author))
                } else {
