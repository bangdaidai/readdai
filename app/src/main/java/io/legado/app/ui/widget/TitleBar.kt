...
//            }
            ...
            // 设置导航图标和溢出菜单图标的颜色
            val textColor = io.legado.app.lib.theme.ThemeStore.titleBarTextIconColor(context)
            setColorFilter(textColor)
            // 强制设置标题文字颜色，覆盖XML中的硬编码
    ...
    fun setColorFilter(@ColorInt color: Int) {
        ...
        // 处理 toolbar 中的 SearchView，使用 post 确保 SearchView 已完成布局
        toolbar.post {
            toolbar.children.forEach { child ->
                if (child is io.legado.app.ui.widget.SearchView) {
                    child.applyTint(color)
                }
            }
        }
    }
...
//        }
    ...
    private fun isInDialog(): Boolean {
        var context = context
        while (context is android.view.ContextThemeWrapper) {
            if (context.javaClass.name.contains("Dialog")) {
                return true
            }
    ...
    private fun attachToActivity() {
        if (attachToActivity) {
            ...
            if (act != null) {
                ...
                act.supportActionBar?.setDisplayHomeAsUpEnabled(displayHomeAsUp)
                // 重新设置颜色过滤器和文字颜色，确保所有元素颜色正确
                val textColor = io.legado.app.lib.theme.ThemeStore.titleBarTextIconColor(context)
                setColorFilter(textColor)
                // 强制设置标题文字颜色，覆盖XML中的硬编码
                toolbar.setTitleTextColor(textColor)
