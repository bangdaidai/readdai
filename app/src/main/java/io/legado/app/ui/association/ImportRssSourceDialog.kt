...
class ImportRssSourceDialog() : BaseDialogFragment(R.layout.dialog_recycler_view),
    ...
    private fun initMenu() {
        ...
        binding.toolBar.menu.apply {
            ...
            // 为自定义源分组菜单项设置正确的颜色
            findItem(R.id.menu_new_group)?.let {
                val tintColor = io.legado.app.lib.theme.ThemeStore.titleBarTextIconColor(requireContext())
                it.title?.let { title ->
    ...
    private fun alertCustomGroup(item: MenuItem) {
        alert(R.string.diy_edit_source_group) {
            ...
            okButton {
                ...
                val title = if (viewModel.groupName.isNullOrBlank()) {
