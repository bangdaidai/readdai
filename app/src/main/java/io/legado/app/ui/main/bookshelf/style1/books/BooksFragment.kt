...
class BooksFragment() : BaseFragment(R.layout.fragment_books),
    ...
    private val booksAdapter: BaseBooksAdapter<*> by lazy {
        when (bookshelfLayout) {
            ...
            2 -> {
                BooksAdapterList3(requireContext(), this, this, viewLifecycleOwner.lifecycle)
            }
            else -> {
    ...
    private fun initRecyclerView() {
        ...
        if (bookshelfLayout >= 3) {
            ...
            // 网格布局：使用 bookshelfMargin 作为左右内边距
            binding.rvBookshelf.setPadding(bookshelfMargin, 0, bookshelfMargin, 0)
        } else {
            binding.rvBookshelf.layoutManager = LinearLayoutManager(context)
            binding.rvBookshelf.setRecycledViewPool(activityViewModel.booksListRecycledViewPool)
            // 列表布局：设置 0dp 左右内边距
            binding.rvBookshelf.setPadding(0, 0, 0, 0)
        }
...
//        })
        binding.rvBookshelf.addItemDecoration( object : RecyclerView.ItemDecoration() {
            override fun getItemOffsets(
                ...
                if (bookshelfLayout >= 3) {
                    val spanCount = bookshelfLayout - 1
                    val rowIndex = position / spanCount
                    when (rowIndex) {
                        0 -> { //第一行加额外上边距
                            outRect.set(bookshelfMargin, bookshelfMargin + 12, bookshelfMargin, bookshelfMargin)
                        }
                        totalRows - 1 -> { //最后一行加额外下边距
                            outRect.set(bookshelfMargin, bookshelfMargin, bookshelfMargin, bookshelfMargin + 12)
                        }
                        else -> {
                    ...
                    when (position) {
                        0 -> {
                            outRect.set(0, bookshelfMargin + 12, 0, bookshelfMargin)
                        }
                        itemCount - 1 -> {
    ...
    private fun upRecyclerData() {
        ...
        booksFlowJob = viewLifecycleOwner.lifecycleScope.launch {
            appDb.bookDao.flowByGroup(groupId).map { list ->
                ...
                itemCount = list.size
                val spanCount = if (bookshelfLayout >= 3) bookshelfLayout - 1 else bookshelfLayout
                if (bookshelfLayout >= 3) {
    ...
    private fun startLastUpdateTimeJob() {
        ...
        if (!AppConfig.showLastUpdateTime || bookshelfLayout >= 3) {
            return
        }
    ...
    override fun observeLiveBus() {
        ...
        observeEvent<String>(EventBus.TAGS_UPDATED) {
            // 标签更新时刷新对应书籍的标签
            // 清除本地缓存，确保显示最新标签
            if (booksAdapter is BooksAdapterList3) {
                (booksAdapter as BooksAdapterList3).clearTagCache(it)
            }
            booksAdapter.notification(it)
        }
