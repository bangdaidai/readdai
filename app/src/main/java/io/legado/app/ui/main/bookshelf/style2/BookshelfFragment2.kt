...
class BookshelfFragment2() : BaseBookshelfFragment(R.layout.fragment_bookshelf2),
    ...
    private val booksAdapterGrid: BooksAdapterGrid by lazy {
        BooksAdapterGrid(requireContext(), this)
    }
    private val booksAdapterList: BooksAdapterList by lazy {
    ...
    private fun initRecyclerView() {
        ...
        if (bookshelfLayout >= 3) {
            ...
            booksAdapter = booksAdapterGrid
            binding.rvBookshelf.layoutManager = GridLayoutManager(context, 4)
            binding.rvBookshelf.setRecycledViewPool(activityViewModel.booksGridRecycledViewPool)
            binding.rvBookshelf.setPadding(bookshelfMargin, 0, bookshelfMargin, 0)
        } else {
            booksAdapter = booksAdapterList
    ...
    override fun upGroup(data: List<BookGroup>) {
        if (data != bookGroups) {
            ...
            val spanCount = when {
                ...
                groupId == BookGroup.IdRoot -> 4
                else -> bookshelfLayout
            }
            if (spanCount >= 2) {
